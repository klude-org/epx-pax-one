...
namespace {(function(){
    ...
    $stf__ft = \filemtime(__FILE__);
    $idx__ft = \filemtime($_SERVER['SCRIPT_FILENAME']);
    $lcl__ft = \is_file($lcl__f = \_\SITE_DIR.'/.local-config.php') ? \filemtime($lcl__f) : 0;
    $cfg__ft = \is_file($cfg__f = \_\SITE_DIR.'/.config.php') ? \filemtime($cfg__f) : 0;
    if($lcl__ft <= $cfg__ft || $lcl__ft <= $idx__ft || $lcl__ft <= $stf__ft){
        $cfg__ft AND include $cfg__f;
        $_ENV['_'] = $_;
        $_ENV['LSP']['LIST'] = \iterator_to_array((function(){
            global $_;
        ...
        $_ENV['THEME'] = $themes;
        $_ENV['TSP']['PATH'] = \implode(PATH_SEPARATOR, \array_keys(\array_filter($modules)));
        $_ENV['site_dir'] = $site_dir = \_\SITE_DIR;
        $_ENV['local_dir'] = \_\SITE_DIR."/.local";
        $_ENV['data_dir'] = ($_['DATA'] ?? false) ? (($MODULE_RESOLVE__FN)($_['DATA']) ?: ((function($p){
            throw new \Exception("Unable to locate data path: '{$p}'");
        }))($_['DATA'])) : (function(){
            \is_dir($d = \_\SITE_DIR."/.local-data") OR \mkdir($d,0777,true);
        })();
        $_ENV['root_dir'] = (function() use(&$root_dir, &$root_url, $site_dir){
        ...
        $_ENV['site_urp'] = $site_urp = (function()use($root_dir, $root_url, $site_dir){
            if(empty($_SERVER['HTTP_HOST'])){
                if($root_url){
        ...
        $_ENV['root_url'] = $root_url;
        $_ENV['site_url'] = $site_url = ($root_url  ? \rtrim($root_url.$site_urp,'/') : "");
